...
.data
	...
	reg1:	.asciiz		"\nEnter a number between 1 and 31 to continue the program:"
	reg2:	.asciiz		"\nCount result is: "
.text
...
RegisterCount:
	...
	syscall
	move $s7, $v0 
	la $s5, main #the first instruction to start from
	la $s6, endProgram
	li $v1, 0
	addi $sp, $sp, -4
	sw $ra, ($sp)
	jal calculateRegisterCount
	jal endOfCalcReg
	lw $ra, ($sp)
	addi $sp, $sp, 4
...
calculateRegisterCount:
	...
	andi 	$s1, $a1, 0xFC000000   # Bit mask the first 6 bits
    	...
    	beq 	$s1, 0x08000000, if_J_instruction
    	beq 	$s1, 0x00000000, if_R_instruction
    	j if_I_instruction
	if_I_instruction:
		andi $s1, $a1, 0x1F0000  	# Mask the bits [20:16]
		srl $s1, $s1, 16                 # Shift the value to the right by 16 bits
		beq $s7, $s1, inc
		j if_I_instruction_else
		inc:
			addi $v1, $v1, 1
		if_I_instruction_else:
		andi $s1, $a1, 0x03E00000 	# Mask the bits [25:21]
		srl $s1, $s1, 21                 # Shift the value to the right by 21 bits
		beq $s7, $s1, if_I_instruction_inc2
		j if_I_instruction_done
		if_I_instruction_inc2:
			addi $v1, $v1, 1
		if_I_instruction_done:
		j continueIteration
	if_R_instruction:	
		andi $s1, $a1, 0xFFE3	  	  # Mask the bits [11:15]
		srl $s1, $s1, 11                 # Shift the value to the right by 16 bits
		beq $s7, $s1, if_R_instruction_inc1
		j if_R_instruction_rtCheck
		if_R_instruction_inc1:
			addi $v1, $v1, 1
		if_R_instruction_rtCheck:
		andi $s1, $a1, 0x1F0000  	# Mask the bits [20:16]
		srl $s1, $s1, 16                 # Shift the value to the right by 16 bits
		beq $s7, $s1, if_R_instruction_Rinc2
		j if_R_instruction_rsCheck
		if_R_instruction_Rinc2:
			addi $v1, $v1, 1
		if_R_instruction_rsCheck:
		andi $s1, $a1, 0x03E00000 	# Mask the bits [25:21]
		srl $s1, $s1, 21                 # Shift the value to the right by 21 bits
		beq $s7, $s1, if_R_instruction_Rinc3
		j if_R_instruction_Rdone
		if_R_instruction_Rinc3:
			addi $v1, $v1, 1
		if_R_instruction_Rdone:
		j continueIteration
	if_J_instruction:
		andi $s1, $a1, 0x3FFFFFF  	 # Mask the bits [25:0]
		sll $s1, $s1, 2                 # Shift the value to the left by 2 bits
		beq $s1, $s7, if_J_instruction_inc
		j if_J_instruction_done
		if_J_instruction_inc:
			addi $v1, $v1, 1
		if_J_instruction_done:
		j continueIteration
	continueIteration:
	addi $s5, $s5, 4
	beqz $a1, end
